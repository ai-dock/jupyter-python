...
All images built for ai-dock are tested for compatibility with both [vast.ai](https://link.ai-dock.org/template-vast-jupyter-python) and [runpod.io](https://link.ai-dock.org/template-runpod-jupyter-python).
...
>[!NOTE]  
>If configured, `sshd`, `caddy`, `cloudflared`, `rclone`, `jupyter`, `port redirector` & `logtail` will be launched before provisioning; Any other processes will launch after.
